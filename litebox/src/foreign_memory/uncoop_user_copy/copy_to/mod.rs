...
use model::copy_to_uncoop_user_model;
use native::copy_to_uncoop_user_native;

verus! {

broadcast use true_tales::amd64::group_amd64_frames;

// Verus cannot capture `dst` mutably, so this helper carries it as explicit context.
pub(super) fn copy_to_uncoop_user_scoped<P>(
    context: (
        ...
        P::thread_invariant(*final(hardware_thread)),
{
    ...
    runtime.copy_to_uncoop_user_with_thread(dst, src, hardware_thread)
}
...
pub(super) fn copy_to_uncoop_user_helper<RM: RmemStack>(
    dst: ForeignPtr,
    src: &[u8],
    ...
    requires
        ...
        old(sess).cap() == dst.cap(),
        UncoopFaultSession::wf(old(sess).st()),
        old(sess).st().region == dst.region(),
        ...
        _extent.spec_contains_range(src@.len() as usize),
        UncoopFaultSession::store_disposition(
            old(sess).st(),
            dst.region(),
            dst.cursor().addr(),
            src@.len() as usize,
        ...
        final(hw).rmem_spec() == old(hw).rmem_spec(),
{
    if cfg!(any(miri, feature = "modeled_backend")) {
        copy_to_uncoop_user_model(dst, src, sess, hw)
    } else {
        copy_to_uncoop_user_native(dst, src, hw)
    }
}
