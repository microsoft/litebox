...
/// 'B'). The test pins:
///   - **No data corruption.** `'B'` bytes in `fd_a` reads must be zero (this is the
///     load-bearing assertion — if it ever fails, the refcount protection regressed).
///   - **No garbage.** Bytes that are neither `'A'` nor `'B'` must be zero.
///
/// Stale-fid `Io` errors are *expected* under this design: if a read's Tread races past
/// `close`'s Tclunk, the server replies Rlerror. The `Io` error count is logged but not
/// asserted — it's a normal symptom of the refcount-not-lock-holding trade-off, where we
/// trade tail latency on `close` for spurious EBADF on racing reads.
#[test]
fn test_nine_p_fid_reuse_race_blocked_by_refcount() {
    ...
    let server = DiodServer::start();
    let fs = connect_9p(&litebox, &server);
    std::fs::write(server.export_path().join("file_a"), [b'A'; FILE_LEN]).unwrap();
    std::fs::write(server.export_path().join("file_b"), [b'B'; FILE_LEN]).unwrap();

    // Open a bunch of throwaway files first to push fid allocation past the
    // initial sequential range, so that file_a's fid lands somewhere where
    // recycling is plausible.
    let mut burn: alloc::vec::Vec<_> = (0..16)
        .map(|_| {
            fs.open("/file_b", OFlags::RDONLY, Mode::empty())
                .expect("open burn fd")
        })
        .collect();
    while let Some(fd) = burn.pop() {
        fs.close(&fd).unwrap();
    }

    let fd_a = fs
        .open("/file_a", OFlags::RDONLY, Mode::empty())
        .expect("open file_a");
    let fd_a = Arc::new(fd_a);

    let detected_b = AtomicUsize::new(0);
    let detected_other = AtomicUsize::new(0);
    let stale_fid_io_errors = AtomicUsize::new(0);
    let close_complete = AtomicBool::new(false);
    let stop = AtomicBool::new(false);

    std::thread::scope(|scope| {
        // Multiple concurrent readers of fd_a. They check every byte — if any
        // byte is 'B' the race corrupted us; any other byte is also wrong.
        for _ in 0..8 {
            let fd_a = Arc::clone(&fd_a);
            let fs = &fs;
            let detected_b = &detected_b;
            let detected_other = &detected_other;
            let stale_fid_io_errors = &stale_fid_io_errors;
            let close_complete = &close_complete;
            let stop = &stop;
            scope.spawn(move || {
                let mut buf = alloc::vec![0u8; FILE_LEN];
                while !stop.load(O::Relaxed) {
                    match fs.read(&fd_a, &mut buf, Some(0)) {
                        Ok(n) => {
                            for &b in &buf[..n] {
                                if b == b'B' {
                                    detected_b.fetch_add(1, O::Relaxed);
                                } else if b != b'A' {
                                    detected_other.fetch_add(1, O::Relaxed);
                                }
                            }
                        }
                        Err(crate::fs::errors::ReadError::ClosedFd) => {
                            // Closer thread has already marked the fd closed
                            // and our `with_entry` returned None. Expected.
                            break;
                        }
                        Err(crate::fs::errors::ReadError::Io) => {
                            // Two ways to get here:
                            //   - real network I/O failure (none expected on
                            //     localhost diod), or
                            //   - server replied Rlerror because the fid we
                            //     extracted has already been clunked by
                            //     another thread. The latter only happens
                            //     after the close has fired.
                            if close_complete.load(O::Relaxed) {
                                stale_fid_io_errors.fetch_add(1, O::Relaxed);
                            }
                        }
                        Err(_) => break,
                    }
        ...
        // Closer + fid churner: wait briefly to let readers ramp up, then
        // close fd_a and stress the fid pool with many open/close cycles on
        // file_b in the hope that one of them recycles fd_a's old fid into
        // file_b just as a concurrent in-flight read is about to send Tread.
        let fd_a = Arc::clone(&fd_a);
        let fs = &fs;
        let close_complete = &close_complete;
        let stop = &stop;
        scope.spawn(move || {
            ...
            let _ = fs.close(&fd_a);
            close_complete.store(true, O::Relaxed);
            for _ in 0..2000 {
                if let Ok(fd_b) = fs.open("/file_b", OFlags::RDONLY, Mode::empty()) {
                    let _ = fs.close(&fd_b);
                }
            }
    ...
    let b = detected_b.load(O::Relaxed);
    let other = detected_other.load(O::Relaxed);
    let stale = stale_fid_io_errors.load(O::Relaxed);
    std::eprintln!(
        "race result: stale-fid Io errors (case B) = {stale}, \
         'B' bytes in file_a reads (case C, data corruption) = {b}, \
         other-non-A bytes = {other}",
    );

    // With refcounted Fid handles, the local pool slot for fd_a's fid stays reserved
    // until every in-flight reader's clone drops, so no new walk can reassign that VALUE
    // to a different file. Data corruption is therefore impossible. Stale-fid Io errors
    // can still occur (close's Tclunk destroyed the server-side fid before our Tread
    // arrived) and are not asserted against.
    assert_eq!(
        b, 0,
        "fid-reuse race regressed: read returned file_b's bytes for fd_a (data corruption) \
         — refcount protection in `Fid` is no longer keeping the pool slot reserved \
         across in-flight reads",
    );
    assert_eq!(
        other, 0,
        "read returned non-A non-B garbage for fd_a (also bad)",
    );
    let _ = stale; // logged above, not asserted
}
