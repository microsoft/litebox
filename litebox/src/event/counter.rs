...
mod tests {
    ...
    fn litebox(responses: impl IntoIterator<Item = CoreResponse>) -> LiteBox<MockPlatform> {
        LiteBox::new_with_broker_control(
            MockPlatform::new(),
            Arc::new(MockBrokerControl::new(responses)),
        )
    }

    #[test]
    fn create_maps_unexpected_response_shape() {
        let litebox = litebox([add_response(false, true)]);
        let result = EventCounter::<MockPlatform>::new(&litebox, 0);
