...
struct LiveCacheMetadata {
    ...
    mappings: Vec<litebox_shim_macos::LiveSharedCacheMapping>,
    slide: usize,
}
...
pub(crate) struct PrivateSharedCacheInstance {
    ...
    pub(crate) mappings: Vec<litebox_shim_macos::LiveSharedCacheMapping>,
...
impl PrivateSharedCacheInstance {
    pub(crate) fn instantiate() -> Result<Self> {
        ...
        let LiveCacheMetadata {
            ...
            mappings,
            slide,
        } = live_cache_metadata(live_base, size)?;
...
fn live_cache_metadata(live_base: usize, live_size: usize) -> Result<LiveCacheMetadata> {
    ...
    let mapped_ranges = mappings
        .iter()
        .map(|mapping| {
            let start = usize::try_from(mapping.address.get(LE))
                .context("shared-cache mapping address overflow")?
                .checked_add(slide)
                .context("slid shared-cache mapping overflow")?;
            let end = start
                .checked_add(
                    usize::try_from(mapping.size.get(LE))
                        .context("shared-cache mapping size overflow")?,
                )
                .context("shared-cache mapping end overflow")?;
            let protection = litebox_common_macos::VmProtection::from_bits(
                i32::try_from(mapping.init_prot.get(LE))
                    .context("shared-cache mapping protection overflow")?,
            )
            .context("invalid shared-cache mapping protection")?;
            Ok(litebox_shim_macos::LiveSharedCacheMapping {
                range: start..end,
                protection,
            })
        })
