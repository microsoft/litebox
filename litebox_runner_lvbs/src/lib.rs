...
/// for slab-backed allocations (the slab needs ≥ 2 MiB backing pages).
///
/// Derives the VTL1 physical base from `get_memory_base_address()` (a linker
/// symbol), so this can be called **before** `parse_boot_info()`.
pub fn seed_initial_heap() {
    ...
    // Add initial 1 MiB heap seed region.
    let mem_fill_start =
...
/// Initialise the current core.
///
/// When `is_bsp` is `true`, creates the platform, sets up page tables,
/// reclaims early memory, and allocates AP per-CPU variables.
/// All cores then initialise hypercalls, GDT, IDT, interrupts, and syscall
/// support.
///
/// # Panics
///
