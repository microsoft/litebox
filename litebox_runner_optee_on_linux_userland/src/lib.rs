...
use litebox::platform::{ThreadLocalStorageProvider, ThreadProvider};
...
mod dispatcher_impl;
use dispatcher_impl::optee_ta::{
    handle_optee_command_output, register_session_id_elf_load_info, session_id_elf_load_info_map,
};
...
pub fn run(cli_args: CliArgs) -> Result<()> {
    ...
    let dispatcher = litebox_runner_command_dispatcher::command_dispatcher();
    dispatcher
        .register_optee_ta_command_handler(dispatcher_impl::optee_ta::optee_ta_command_handler);
    dispatcher.register_return_function(return_function_to_command_dispatcher);
...
pub fn replay_ta_commands(session_id: u32) -> ! {
    ...
    if let Some(cmd) = optee_command_replay_queue().pop(session_id) {
        dispatcher.handle_optee_ta_command(&cmd);
    } else {
...
/// OP-TEE TA command (base64 encoded) in replay log. It consists of a function ID,
...
/// An argument of an OP-TEE TA command (base64 encoded). It consists of
...
/// A function to return from an OP-TEE TA command handler. This function
/// handles the command output (if exists) and returns the one which interact with
/// the command dispatcher like a client or a replayer (this crate's case).
pub fn return_function_to_command_dispatcher() -> ! {
    let tid = litebox_platform_multiplex::platform()
        .with_thread_local_storage_mut(|tls| tls.current_task.tid);
    #[allow(clippy::cast_sign_loss)]
    let session_id = tid as u32;
