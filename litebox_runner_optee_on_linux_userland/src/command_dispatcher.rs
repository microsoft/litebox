...
pub fn optee_ta_command_handler(command: &OpteeTaCommand) {
    ...
    let mut pt_regs = PtRegs {
        ...
        r8: 0,
        rax: 0,
        rcx: usize::try_from(cmd_id).unwrap(),
        rdx: elf_load_info.params_address,
        rsi: usize::try_from(session_id).unwrap(),
        rdi: usize::try_from(func as u64).unwrap(),
        orig_rax: 0,
        rip: elf_load_info.entry_point,
        cs: 0x33, // __USER_CS
        eflags: 0,
