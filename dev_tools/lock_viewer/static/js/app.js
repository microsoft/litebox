...
function calculateCommonPrefix() {
    ...
    if (files.length === 1) {
        // For single file, strip everything up to the last directory
    ...
    for (let i = 1; i < files.length; i++) {
        while (!files[i].startsWith(prefix) && prefix.length > 0) {
            // Remove one character at a time from the right
...
function updateLockFilter() {
    ...
    const lockArray = Array.from(uniqueLocks).sort((a, b) => {
        const contentionA = lockContention[a]?.totalWait || 0;
        const contentionB = lockContention[b]?.totalWait || 0;
        return contentionB - contentionA;  // Descending order
    });
    ...
    lockArray.forEach(lock => {
        ...
        // Show lock address with contention info
        const cont = lockContention[lock];
        let contentionInfo = '';
        if (cont && cont.totalWait > 0) {
            contentionInfo = ` (${formatDuration(cont.totalWait)})`;
        }
        tag.textContent = lock + contentionInfo;
        tag.title = cont ? 
            `Total wait: ${formatDuration(cont.totalWait)}\nAttempts: ${cont.attempts}\nMax wait: ${formatDuration(cont.maxWait)}` :
            'No contention data';
...
function renderTimeline() {
    ...
    if (selectedLocks.size === 0) {
        ...
        document.getElementById('timeline-content').innerHTML = `
            <div class="no-data welcome-message">
                ...
                <p>Click on lock tags above to enable them, or use "Select All" to show everything.</p>
                <p class="muted">${totalLocks} locks available • Locks are sorted by total contention time</p>
            </div>
        `;
        return;
    }
    ...
    sortedLocks.forEach(lock => {
        ...
        const cont = lockContention[lock];
        const contentionBadge = (cont && cont.totalWait > 0) ? 
            `<span class="contention-badge">${formatDuration(cont.totalWait)}</span>` : '';
        const label = `${lock} (${strippedFile}:${firstEvent.line})`;
        const fullLabel = `${lock} (${firstEvent.file}:${firstEvent.line})`;

        html += `<div class="timeline-track">`;
        html += `<div class="timeline-track-label" title="${fullLabel}">${label}${contentionBadge}</div>`;
