...
broadcast use crate::amd64::group_amd64_frames;

pub fn drive_fdomain_end_to_end(obj: StableUserDomainState)
    requires
        obj.wf(),
{
    ...
    let region_arg = obj.region();
    let (_id, _ptr, h) = fmem.inject_domain_ptr::<StableUserDomain>(region_arg, obj);

    hw.cpu.bind_reg(Reg::Rdi, RegVal::ForeignMemPtr(_ptr));
    assert(hw.cpu_spec().get_reg_spec(Reg::Rdi) == RegVal::ForeignMemPtr(_ptr));
    assert(_ptr.domain() == _id);

    assert(fmem.contains(_id));
    assert(fmem.obj_tag(_id) == type_tag::<UnsizeShim<StableUserDomainState>>());
    ...
    let data: Vec<u8> = vec![0x3cu8, 0xc3u8];
    ...
    hw.reg_int_load_imm64(Reg::Rax, 40);
    hw.reg_int_add_imm64(Reg::Rax, 2);
    assert(hw.cpu_spec().get_reg_spec(Reg::Rax) == RegVal::Int(42));
    ...
    assert(_readback@ == data@);
    assert(_readback@ == seq![0x3cu8, 0xc3u8]);
...
#[test]
fn fdomain_round_trips_through_the_thread() {
    let region = ForeignRegionId { raw: 7 };
    let cookie = DomainCookie { raw: 3 };
    let obj = StableUserDomainState::zeroed(region, 0, 8, cookie);

    drive_fdomain_end_to_end(obj);
}
