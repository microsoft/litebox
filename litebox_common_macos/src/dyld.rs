...
//! Preparation of dyld for use with an externally supplied shared cache.
//!
...
use crate::{
    VmProtection,
    loader::{MachoLoaderError, MachoParsedFile, arm64_slice_range},
};
use core::ops::Range;
...
/// Patch an unrewritten dyld image to remain in its private mapping while using
/// an externally supplied shared cache whose initializers have already run.
pub fn patch_for_initialized_shared_cache(data: &mut [u8]) -> Result<(), MachoLoaderError> {
    let slice_range = arm64_slice_range(data)?;
    let image = &mut data[slice_range];
    let mut plan = MachoParsedFile::parse(image)?;
    if !plan.is_dyld {
        return Err(MachoLoaderError::Invalid("supplied image is not dyld"));
    }
    match plan.parse_trampoline(image) {
        Err(MachoLoaderError::Unrewritten) => {}
        Ok(_) => return Err(MachoLoaderError::Unsupported("pre-rewritten dyld")),
        Err(error) => return Err(error),
    }
    let executable_ranges: alloc::vec::Vec<_> = plan
        .segments
        .iter()
        .filter(|segment| segment.protection.contains(VmProtection::EXECUTE))
        .map(|segment| segment.file_range.clone())
        .collect();
    patch(image, &executable_ranges)
}

fn patch(data: &mut [u8], executable_ranges: &[Range<usize>]) -> Result<(), MachoLoaderError> {
