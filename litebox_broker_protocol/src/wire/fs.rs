...
use crate::fs::{
    ...
    FilesystemFileStatus, FilesystemMode, FilesystemNamespace, FilesystemNodeInfo,
    FilesystemOpenFlags, FilesystemSeekWhence, FilesystemUser, HandleFileStatusRequest,
...
fn decode_mode(decoder: &mut Decoder<'_>) -> Result<FilesystemMode, WireError> {
    FilesystemMode::from_bits(decoder.u16()?).ok_or(WireError::InvalidTag)
}
...
pub(super) fn decode_fs_request(decoder: &mut Decoder<'_>) -> Result<FilesystemRequest, WireError> {
    match decoder.u8()? {
        REQUEST_TAG_OPEN => Ok(FilesystemRequest::Open(OpenFileRequest {
            ...
            flags: FilesystemOpenFlags::from_bits(decoder.u16()?).ok_or(WireError::InvalidTag)?,
...
fn encode_access_mode(encoder: &mut Encoder, access: FilesystemAccessMode) {
    encoder.u8(match access {
        FilesystemAccessMode::ReadOnly => 0,
        FilesystemAccessMode::WriteOnly => 1,
        FilesystemAccessMode::ReadWrite => 2,
    });
}

fn decode_access_mode(decoder: &mut Decoder<'_>) -> Result<FilesystemAccessMode, WireError> {
    match decoder.u8()? {
        0 => Ok(FilesystemAccessMode::ReadOnly),
        1 => Ok(FilesystemAccessMode::WriteOnly),
        2 => Ok(FilesystemAccessMode::ReadWrite),
        _ => Err(WireError::InvalidTag),
    }
}
