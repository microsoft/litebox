...
/// An active macOS-userland broker association.
...
/// Connects to and negotiates an association with a macOS-userland broker.
pub fn connect(control_socket_path: &Path) -> Result<BrokerConnection> {
    ...
    let (local, notification_channel) = BrokerLocal::negotiate(setup_channel, |mut setup| {
        let shared_memory =
            setup.receive_shared_memory(SHARED_BUFFER_POOL_SIZE, Some(setup_deadline))?;
...
mod tests {
    ...
    fn activate_control_channel(
        ...
        let local_memory = MacosSharedMemory::create_control_ring().unwrap();
        let host_memory = MacosSharedMemory::control_ring_from_received_fd(
