...
use rustix::io::Errno;
use rustix::net::{AddressFamily, SocketAddrUnix, SocketType, connect, socket, sockopt};
...
use crate::shared_memory::MacosSharedMemory;
...
struct LocalRingAssociation {
    request_producer: Mutex<ControlRingProducer<MacosSharedMemory>>,
    control_stream: UnixStream,
    pending_calls: Arc<PendingCalls>,
    on_failure: Arc<dyn Fn() + Send + Sync>,
    request_wake: ControlRingWakeHandle<MacosSharedMemory>,
    response_wake: ControlRingWakeHandle<MacosSharedMemory>,
    notification_wake: ControlRingWakeHandle<MacosSharedMemory>,
}

/// Local notification receiver for a shared-ring Unix broker association.
pub struct UnixControlRingLocalNotificationChannel {
    consumer: ControlRingConsumer<MacosSharedMemory>,
    association: Arc<LocalRingAssociation>,
}
...
impl UnixStreamLocalSetupChannel {
    ...
    /// Receives one POSIX shared-memory object offered by the broker during setup.
    pub fn receive_shared_memory(
        &mut self,
        expected_len: usize,
        deadline: Option<Instant>,
    ) -> IoResult<MacosSharedMemory> {
        crate::shared_memory::receive_shared_memory(&mut self.stream, expected_len, deadline)
    }

    /// Receives the control-ring POSIX shared-memory object offered by the broker during setup.
    pub fn receive_control_ring(
        &mut self,
        deadline: Option<Instant>,
    ) -> IoResult<MacosSharedMemory> {
        crate::shared_memory::receive_control_ring(&mut self.stream, deadline)
    }
    ...
    ///
    /// The ring must be the validated control-ring POSIX shared-memory object received during this
    /// setup exchange.
    pub fn into_active(
        self,
        ring: ControlRing<MacosSharedMemory>,
...
fn connect_with_deadline(path: &Path, deadline: Instant) -> IoResult<UnixStream> {
    ...
    loop {
        ...
        match connect(&socket, &address) {
            ...
            Err(Errno::INTR) => {}
            // Darwin reports both a full Unix listen queue and a stale socket
            // as ECONNREFUSED. No connection has started, so retry rather than
            // poll. These cases cannot be distinguished here: a stale socket
            // also gets retried, bounded by the absolute setup deadline.
            Err(Errno::AGAIN | Errno::CONNREFUSED) => {
...
fn nonblocking_socket() -> IoResult<OwnedFd> {
...
impl LocalRingAssociation {
    fn acknowledge_notification(
        &self,
        consumer: &mut ControlRingConsumer<MacosSharedMemory>,
    ) -> IoResult<()> {
        let result = self
            .pending_calls
            .run_if_live(|| {
...
fn dispatch_responses(
    mut consumer: ControlRingConsumer<MacosSharedMemory>,
    association: Arc<LocalRingAssociation>,
) {
    loop {
...
mod control_ring_tests {
    ...
    type Producer = ControlRingProducer<MacosSharedMemory>;
    type Consumer = ControlRingConsumer<MacosSharedMemory>;
    ...
    #[test]
    fn socket_paths_are_private_and_cleaned_up() {
        let first = TestSocketPath::new();
        let second = TestSocketPath::new();
        let directory = first.as_path().parent().unwrap().to_owned();
        assert_ne!(directory, second.as_path().parent().unwrap());
        assert_eq!(
            std::fs::metadata(&directory).unwrap().permissions().mode() & 0o777,
            0o700
        );
        drop(UnixListener::bind(first.as_path()).unwrap());
    ...
    fn saturated_listener() -> (TestSocketPath, UnixListener, Vec<std::os::fd::OwnedFd>) {
        ...
        for _ in 0..1024 {
            let socket = nonblocking_socket().unwrap();
            match connect(&socket, &address) {
                Ok(()) => queued.push(socket),
                Err(Errno::AGAIN | Errno::CONNREFUSED) => return (path, listener, queued),
    ...
    fn ring_pair() -> (
        ControlRing<MacosSharedMemory>,
        ControlRing<MacosSharedMemory>,
    ) {
        let first = MacosSharedMemory::create_control_ring().unwrap();
        let second = MacosSharedMemory::control_ring_from_received_fd(
    ...
    #[test]
    fn stale_socket_retries_until_the_setup_deadline() {
        let path = TestSocketPath::new();
        drop(UnixListener::bind(path.as_path()).unwrap());
        assert!(path.as_path().exists());
        let deadline = Instant::now() + Duration::from_millis(50);
        let Err(error) =
            UnixStreamLocalSetupChannel::connect_with_setup_deadline(path.as_path(), deadline)
        else {
            panic!("connected to a stale socket");
        };
        assert_eq!(error.kind(), ErrorKind::TimedOut);
