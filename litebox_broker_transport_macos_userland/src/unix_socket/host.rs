...
fn peer_process_id(stream: &UnixStream) -> IoResult<u32> {
    let expected_length = size_of::<libc::pid_t>();
    let mut process_id: libc::pid_t = 0;
    let mut actual_length = libc::socklen_t::try_from(expected_length).unwrap();
    // SAFETY: the socket is live and both output pointers identify writable
    // storage of the sizes required by LOCAL_PEERPID.
    let result = unsafe {
        libc::getsockopt(
            stream.as_raw_fd(),
            libc::SOL_LOCAL,
            libc::LOCAL_PEERPID,
            std::ptr::from_mut(&mut process_id).cast(),
    ...
    validate_peer_process_id(process_id)
}
...
pub struct UnixControlRingHostResponseSink {
    producer: Arc<Mutex<ControlRingProducer<MacosSharedMemory>>>,
    association: Arc<HostRingAssociation>,
}
...
pub struct UnixControlRingHostNotificationChannel {
    producer: ControlRingProducer<MacosSharedMemory>,
    association: Arc<HostRingAssociation>,
}
...
impl UnixStreamHostSetupChannel {
    ...
    /// Sends a POSIX shared-memory object during association setup.
    pub fn send_shared_memory(
        &mut self,
        shared_memory: &MacosSharedMemory,
        deadline: Option<Instant>,
    ) -> IoResult<()> {
        crate::shared_memory::send_shared_memory(&mut self.stream, shared_memory, deadline)
    }
...
impl HostRingAssociation {
    ...
    fn try_publish(
        &self,
        producer: &mut ControlRingProducer<MacosSharedMemory>,
        frame: &[u8],
...
mod tests {
    ...
    #[test]
    fn macos_peer_validation_identifies_connected_process() {
    ...
    #[test]
    fn macos_peer_validation_rejects_unavailable_process_ids() {
