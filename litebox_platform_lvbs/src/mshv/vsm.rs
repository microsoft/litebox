...
pub fn mshv_vsm_load_kdata(pa: u64, nranges: u64) -> Result<i64, Errno> {
    ...
    serial_println!("VSM: ***About to load system certs");
    ...
    serial_println!("VSM: ***Loaded system certs");
    ...
    serial_println!("VSM: ***loaded kexec");
    ...
    serial_println!("VSM: ***Loaded patches");
    ...
    serial_println!("VSM: ***Got kinfo_mem and ksdata_mem");
    ...
    kinfo_mem
        ...
        .unwrap();
serial_println!("VSM: ***Gread kinfo_mem");
        //.map_err(|_| Errno::EINVAL)?;
    //let kinfo = HekiKernelInfo::from_bytes(&kinfo_buf)?;
    vtl0_info.kinfo.from(&kinfo_buf).unwrap();
    serial_println!("VSM: ***Read kinfo buf");
    let Some((ksymtab_start, ksymtab_end, ksymtab_gpl_start, ksymtab_gpl_end)) =
        vtl0_info.kinfo.get_sym_info()
    else {
        serial_println!("!!!get_sym_info failed");
        return Err(Errno::EINVAL);
    };
    ...
    pub fn find_symbol(&self, sym_name: &str) -> Option<u64> {
        match self.gpl_symbols.find(sym_name) {
            None => match self.symbols.find(sym_name) {
                None => self.module_memory_metadata.find_symbol(sym_name),
...
impl ModuleMemoryMetadata {
    ...
    pub(crate) fn find_symbol(&self, sym_name: &str) -> Option<u64> {
        match self.gpl_symbols.find(sym_name) {
            ...
            //    Some(value) => Some(value),
            //},
            None => self.symbols.find(sym_name),
            Some(value) => Some(value),
        }
    }
}
...
impl SymbolTable {
    ...
    pub fn build_from_container(
        ...
        for _ in 0..ksym_count {
            ...
            if rate_limit < 20 {
                crate::serial_println!("    sym:{} value:{}", name, sym.value);
                rate_limit += 1;
            }
    ...
    pub fn list_one(&self) -> Option<String> {
        ...
        //    return Some(s.clone());
       // }
        if let Some((s, _v)) = inner.iter().next() {
            return Some(s.clone())
        }
        None
    }
...
pub struct KernelInfo {
    inner: spin::rwlock::RwLock<Option<HekiKernelInfo>>,
}

impl KernelInfo {
    pub fn new() -> Self {
        Self {
            inner: spin::rwlock::RwLock::new(None),
        }
    }

    pub fn from(&self, bytes: &[u8]) -> Result<u64, Errno> {
        let kinfo = HekiKernelInfo::from_bytes(bytes)?;
        let mut inner = self.inner.write();
        *inner = Some(kinfo);
        Ok(0)
    }
    ...
    pub fn get_sym_info(
        ...
        let inner = self.inner.read();
        if let Some(kinfo) = inner.as_ref() {
            let (a, b, c, d) = kinfo.get_sym_info();
            Some((a, b, c, d))
        } else {
            None
        }
    ...
    pub fn get_arch_kinfo(&self) -> Option<(u64, u64, u64, u64, u64)> {
        let inner = self.inner.read();
        //if let Some(kinfo) = inner.as_ref() {
        //    Some(kinfo.get_arch_kinfo())
        //} else {
        //    None
        //}
        inner.as_ref().map(super::heki::HekiKernelInfo::get_arch_kinfo)
    }

    pub fn get_arch_pv_op(&self, index: usize) -> Option<u64> {
        let inner = self.inner.read();
        if let Some(kinfo) = inner.as_ref() {
            kinfo.get_arch_pv_op(index)
        } else {
            None
        }
    }
