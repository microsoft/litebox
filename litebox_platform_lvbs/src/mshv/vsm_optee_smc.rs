...
use crate::{host::per_cpu_variables::with_per_cpu_variables_mut, mshv::HV_VTL_SECURE};
...
/// We translate SMCCC into VTL switch convention. Specifically, we reserve a page in VTL0's memory
/// to exchange OP-TEE SMC call arguments and results (up to nine 64-bit values).
...
impl OpteeSmcArgs {
    ...
    /// Get the physical address of `OpteeMsgArg` from VTL0's memory using OP-TEE SMC call arguments.
    ...
    /// Get the physical address of `OpteeMsgArg` from VTL0's "registered" memory using OP-TEE SMC call arguments.
    ...
    /// Get `OpteeMsgArg` and its physical address from VTL0's memory using OP-TEE SMC call arguments.
    #[expect(dead_code)]
    ...
    /// Set `OpteeMsgArg` into VTL0's memory using OP-TEE SMC call arguments to return the result.
...
pub(crate) fn optee_smc_dispatch(optee_smc_args_pfn: u64) -> i64 {
    let Ok(optee_smc_args_page_addr) = PhysAddr::try_new(optee_smc_args_pfn << 12) else {
    ...
    if let Ok(func_id) = optee_smc_args.func_id() {
        match func_id {
            ...
            | OpteeSmcFunction::CallWithRegdArg => {
                ...
                crate::debug_serial_println!("TODO: TA command invocation");
    ...
    // Return the result by writing back the updated OP-TEE SMC arguments to VTL0's memory
    if unsafe {
        crate::platform_low()
