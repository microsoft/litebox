...
/// This function behaves like C memcpy.
/// # Safety
/// `dest` and `src` must be valid for reads/writes of `count` bytes, unless
/// `count == 0`, in which case no memory is touched and `dest` is returned.
/// This function is only for debug builds to avoid erroneous safety checks of
/// debug build of `compiler-builtins-mem'.
...
/// This function behaves like C memcmp.
/// # Safety
/// if `count == 0` returns 0 without touching memory; otherwise `lhs` and
/// `rhs` must be valid for reads of `count` bytes.
/// This function is only for debug builds to avoid erroneous safety checks of
/// debug build of `compiler-builtins-mem'.
#[cfg(debug_assertions)]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn memcmp(lhs: *const u8, rhs: *const u8, count: usize) -> i32 {
    ...
    if count == 0 {
        ...
        let lhs_rem = unsafe { core::ptr::read(lhs.offset(-1)) };
        let rhs_rem = unsafe { core::ptr::read(rhs.offset(-1)) };
        i32::from(lhs_rem) - i32::from(rhs_rem)
    }
}
