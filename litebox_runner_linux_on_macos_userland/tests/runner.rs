...
fn accepts_4k_loads_sharing_a_native_page() {
    let fixture = Fixture::new();
    let mut binary = elf(EXIT_42);
    // RO headers and RX text occupy different 4 KiB subpages of one host page.
    // Text is congruent at 4 KiB, but not at 16 KiB.
    binary[56..58].copy_from_slice(&2u16.to_le_bytes());
    binary[24..32].copy_from_slice(&0x2000u64.to_le_bytes());
    phdr(&mut binary[64..120], 1, 4, 0, 0, 0x1000, 0x1000, 0x1000);
    phdr(
        &mut binary[120..176],
        1,
        5,
        0x1000,
        0x2000,
        (EXIT_42.len() * 4) as u64,
        (EXIT_42.len() * 4) as u64,
        0x1000,
    );
    let shoff = usize::try_from(u64::from_le_bytes(binary[40..48].try_into().unwrap())).unwrap();
    binary[shoff + 64 + 16..shoff + 64 + 24].copy_from_slice(&0x2000u64.to_le_bytes());
    std::fs::write(fixture.0.join("program"), binary).unwrap();
    let output = fixture.run(&[]);
...
fn executes_and_writes_mixed_subpages_from_another_native_page() {
    let fixture = Fixture::new();
    let code = [
        0x10020013, // adr x19, data (0x5000 from PC 0x1000)
        0x94000bff, // bl helper (0x4000 from PC 0x1004)
        0xb9000260, // str w0, [x19] -- changes the helper's native page to RW
        0x94000bfd, // bl helper (0x4000 from PC 0x100c) -- back to RX
        0xb9000260, // str w0, [x19] -- back to RW
        0xd2800540, 0xd2800ba8, 0xd4000001, // exit(42)
    ];
    let mut binary = elf(&code);
    binary[56..58].copy_from_slice(&3u16.to_le_bytes());
    phdr(
        &mut binary[120..176],
        1,
        5,
    ...
    binary.resize(0x2000, 0);
    for instruction in [0xd2800540u32, 0xd65f03c0] {
        // mov x0, #42; ret
...
fn same_native_page_store_faults_instead_of_looping() {
    let fixture = Fixture::new();
    let code = [
        0x10008001, // adr x1, data (0x2000 from PC 0x1000)
        0x52800540, // mov w0, #42
        0xb9000020, // str w0, [x1] -- instruction and data share a native page
        0xd2800ba8, 0xd4000001,
    ];
    let mut binary = elf(&code);
    binary[56..58].copy_from_slice(&2u16.to_le_bytes());
    phdr(
    ...
    std::fs::write(fixture.0.join("program"), binary).unwrap();
    let output = fixture.run(&[]);
    assert_eq!(output.status.code(), Some(128 + 11), "{output:?}");
}
