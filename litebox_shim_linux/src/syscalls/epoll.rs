...
pub(crate) enum EpollDescriptor {
    ...
    Socket(litebox::net::socket_channel::UserNetworkProxy<Platform>),
...
impl EpollDescriptor {
    pub fn try_from(files: &FilesState, desc: &Descriptor) -> Result<Self, Errno> {
        match desc {
            ...
            Descriptor::Socket { user, .. } => {
                Ok(EpollDescriptor::Socket(user.clone()))
            }
        }
    }
}
...
enum DescriptorRef {
    ...
    Socket(litebox::net::socket_channel::WeakUserNetworkProxy<Platform>),
...
impl DescriptorRef {
    fn from(value: &EpollDescriptor) -> Self {
        match value {
            ...
            EpollDescriptor::Socket(proxy) => Self::Socket(proxy.downgrade()),
...
impl EpollEntryKey {
    fn new(fd: u32, desc: &EpollDescriptor) -> Self {
        let ptr = match desc {
            ...
            EpollDescriptor::Socket(proxy) => proxy.addr(),
