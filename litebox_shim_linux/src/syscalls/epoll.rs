...
pub(crate) enum EpollDescriptor {
    ...
    Timerfd(Arc<super::timerfd::TimerFile<Platform>>),
}
...
impl EpollDescriptor {
    pub fn try_from(files: &FilesState, desc: &Descriptor) -> Result<Self, Errno> {
        match desc {
            ...
            Descriptor::Timerfd { file, .. } => Ok(EpollDescriptor::Timerfd(file.clone())),
        }
    }
}
...
enum DescriptorRef {
    ...
    Timerfd(Weak<crate::syscalls::timerfd::TimerFile<Platform>>),
}
...
impl DescriptorRef {
    fn from(value: &EpollDescriptor) -> Self {
        match value {
            ...
            EpollDescriptor::Timerfd(timerfd) => Self::Timerfd(Arc::downgrade(timerfd)),
        }
    }
...
impl EpollDescriptor {
    ...
    fn poll(
        ...
        let io_pollable: &dyn IOPollable = match self {
            ...
            EpollDescriptor::Timerfd(file) => file,
        };
...
impl EpollEntryKey {
    fn new(fd: u32, desc: &EpollDescriptor) -> Self {
        let ptr = match desc {
            ...
            EpollDescriptor::Timerfd(timerfd) => Arc::as_ptr(timerfd).addr(),
        };
        Self(fd, ptr)
    }
}
