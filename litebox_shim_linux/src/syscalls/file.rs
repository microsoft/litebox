...
use litebox_common_linux::{
    AtFlags, EfdFlags, EpollCreateFlags, FcntlArg, FileDescriptorFlags, FileStat, FsId, IoReadVec,
    IoWriteVec, IoctlArg, Statfs, TimeParam, errno::Errno,
};
...
impl Task {
    ...
    /// Helper function to create a synthetic `Statfs` result.
    ///
    ...
    fn make_statfs(&self) -> Statfs {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            ...
            Statfs {
                f_type: TMPFS_MAGIC,
                f_bsize: BLOCK_SIZE,
                f_blocks: 1024 * 1024 * 1024, // ~4 PB total
                f_bfree: 1024 * 1024 * 1024,  // All free
                f_bavail: 1024 * 1024 * 1024, // All available
                f_files: 1024 * 1024,         // 1M inodes
                f_ffree: 1024 * 1024,         // All free
                f_fsid: FsId { val: [1, 0] }, // Synthetic filesystem ID
    ...
    ///
    /// Returns information about a mounted filesystem. The `path` argument
    /// specifies the pathname of any file within the mounted filesystem.
    pub fn sys_statfs(&self, path: impl path::Arg) -> Result<Statfs, Errno> {
        // Verify the path exists by attempting to stat it
        let _ = self.do_stat(path, true)?;
        Ok(self.make_statfs())
    }

    /// Handle syscall `fstatfs`
    ///
    /// Returns information about a mounted filesystem. The file descriptor `fd`
    /// refers to an open file within the mounted filesystem.
    pub fn sys_fstatfs(&self, fd: i32) -> Result<Statfs, Errno> {
        // Verify the file descriptor is valid by attempting to stat it
        let _ = self.sys_fstat(fd)?;
        Ok(self.make_statfs())
    }
