    ...
    fn setsockopt(
        ...
        match optname {
            SocketOptionName::IP(ip) => match ip {
                // IP_TOS is an advisory traffic-class hint. Accept it silently
                // (we don't propagate the bit anywhere) instead of returning
                // EOPNOTSUPP, which Node's Socket.setTypeOfService treats as
                // fatal and cascades into tearing down the connection.
            ...
            SocketOptionName::Socket(so) => match so {
                ...
                // and keep the fixed internal buffer size that getsockopt
                // reports, instead of returning EOPNOTSUPP (which Node's TLS
                // socket path treats as fatal).
