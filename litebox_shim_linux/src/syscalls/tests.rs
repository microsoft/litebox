...
fn test_getdent64() {
    ...
    while offset < bytes_read {
        let dirent_ptr =
            unsafe { buffer.as_ptr().add(offset) as *const litebox_common_linux::LinuxDirent64 };
    ...
    if bytes > 0 {
        let dirent_ptr = small_buffer.as_ptr() as *const litebox_common_linux::LinuxDirent64;
        let dirent = unsafe { &*dirent_ptr };
        assert!(
    ...
    let file_fd = sys_open("/test_file1.txt", OFlags::RDONLY, Mode::empty())
        .expect("Failed to open test_file1.txt");
    let file_fd = file_fd.try_into().unwrap();
    ...
    super::file::sys_close(file_fd).expect("Failed to close file");
    ...
    loop {
        ...
        while offset < bytes_read {
            let dirent_ptr = unsafe {
                chunk_buffer.as_ptr().add(offset) as *const litebox_common_linux::LinuxDirent64
            };
            let dirent = unsafe { &*dirent_ptr };
