...
#[test]
fn duplicated_directory_descriptors_share_position() {
    fn read_names(
        ...
        while offset < bytes_read {
            ...
            let name_end = offset + usize::from(dirent.len);
            let name_bytes = &buffer[name_start..name_end];
            let name_len = name_bytes.iter().position(|&byte| byte == 0).unwrap();
            names.push(
                core::str::from_utf8(&name_bytes[..name_len])
                    .unwrap()
                    .into(),
            );
            offset = name_end;
    ...
    let task = init_platform(None);
    let dir_fd = i32::try_from(task.sys_open("/", OFlags::RDONLY, Mode::empty()).unwrap()).unwrap();
    let dup_fd = i32::try_from(task.sys_dup(dir_fd, None, None).unwrap()).unwrap();

    assert_eq!(read_names(&task, dir_fd), alloc::vec![".", ".."]);
    assert_eq!(
        task.sys_lseek(dup_fd, 0, SeekWhence::RelativeToCurrentOffset),
        Ok(2)
    );
    assert_eq!(read_names(&task, dup_fd), alloc::vec!["bar", "dev"]);
    assert_eq!(
        task.sys_lseek(dir_fd, 0, SeekWhence::RelativeToCurrentOffset),
        Ok(4)
    );

    assert_eq!(
        task.sys_lseek(dup_fd, 0, SeekWhence::RelativeToBeginning),
        Ok(0)
    );
    assert_eq!(
        task.sys_lseek(dir_fd, 0, SeekWhence::RelativeToCurrentOffset),
