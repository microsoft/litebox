...
#[test]
fn test_fcntl_lock_and_unlock() {
    init_platform(None);

    // Open a regular file read-write; create if missing.
    let fd = sys_open(
        "/lock_test_file.txt",
    ...
    // 1) Take an exclusive (write) lock for the whole file via SETLK.
    let mut lock_set = litebox_common_linux::Flock {
        type_: litebox_common_linux::FlockType::WriteLock as i16,
        whence: super::file::SEEK_SET,
        start: 0,
        // len = 0 means to EOF.
        len: 0,
        pid: 0,
    ...
    {
        use litebox::platform::RawConstPointer as _;
        let ptr = <litebox_platform_multiplex::Platform as litebox::platform::RawPointerProvider>::RawConstPointer::<
            litebox_common_linux::Flock,
        >::from_usize(&raw const lock_set as usize);
        assert_eq!(
            sys_fcntl(fd, litebox_common_linux::FcntlArg::SETLK(ptr)),
            Ok(0)
        );
    }

    // 2) Query with GETLK for a READ lock over the same range; same owner should not conflict.
    let mut probe = litebox_common_linux::Flock {
    ...
    {
        let probe_ptr = <litebox_platform_multiplex::Platform as litebox::platform::RawPointerProvider>::RawMutPointer::<
            litebox_common_linux::Flock,
        >::from_usize(&raw mut probe as usize);
        assert_eq!(
            sys_fcntl(fd, litebox_common_linux::FcntlArg::GETLK(probe_ptr)),
            Ok(0)
        );
        // No conflict expected (same owner), so kernel/shim sets type_ = Unlock.
        assert_eq!(
            probe.type_,
            litebox_common_linux::FlockType::Unlock as i16,
            "GETLK should report no conflict (Unlock) for same-owner locks"
        );
    }

    // 3) Unlock the whole file.
    let mut unlock = litebox_common_linux::Flock {
        type_: litebox_common_linux::FlockType::Unlock as i16,
        whence: 0,
        start: 0,
        len: 0,
        pid: 0,
    };
    {
        let ptr = <litebox_platform_multiplex::Platform as litebox::platform::RawPointerProvider>::RawConstPointer::<
            litebox_common_linux::Flock,
        >::from_usize(&raw const unlock as usize);
        assert_eq!(
            sys_fcntl(fd, litebox_common_linux::FcntlArg::SETLK(ptr)),
            Ok(0)
        );
