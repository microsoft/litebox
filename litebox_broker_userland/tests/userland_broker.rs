...
// `harness = false`: this file is the test binary's entry point, so it keeps a
// `main` on every host and configures only its Linux body away.
#[cfg(target_os = "linux")]
mod linux_test {
...
fn main() {
    eprintln!("The LiteBox userland broker tests are only supported on Linux");
}
