...
impl<'a> Image<'a> {
    ...
    fn map<P: ShimPlatform>(&self, task: &Task<P>) -> Result<MappedImage, MachoLoaderError> {
        ...
        for segment in &self.plan.segments {
            ...
            // Standalone dyld mutates its DATA_CONST/authenticated bookkeeping
...
pub(super) fn load<P: ShimPlatform>(
    ...
    if let Some(dyld) = &dyld {
        task.global
            ...
            .store(dyld.header, core::sync::atomic::Ordering::Release);
        task.global
            .standalone_dyld_end
            .store(dyld.end, core::sync::atomic::Ordering::Release);
    }
    let mut apple = vec![
