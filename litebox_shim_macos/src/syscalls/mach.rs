...
use litebox_common_macos::{
    ...
    errno::Errno,
    syscall::{
        MachMessageOptions, MachPortName, MachTimebaseInfo, MachVmAddressMask, MachVmFlags,
        MachVmProtection, synthetic_port,
    },
...
impl<P: ShimPlatform> Task<P> {
    ...
    /// Return a synthetic receive-right name used only as a dyld identity.
    pub(crate) const fn synthetic_reply_port() -> MachPortName {
        synthetic_port::REPLY
    }

    /// Return the synthetic port name for the current guest thread.
    pub(crate) const fn synthetic_thread_port() -> MachPortName {
        synthetic_port::THREAD_SELF
    }

    /// Return the synthetic port name for the guest task.
    pub(crate) const fn synthetic_task_port() -> MachPortName {
        synthetic_port::TASK_SELF
    }

    /// Return the synthetic port name for the shim's host endpoint.
    ...
    pub(crate) fn sys_mach_msg2_compat(options: MachMessageOptions) -> usize {
        const MACH_RCV_TIMED_OUT: usize = 0x1000_4003;
        if options.contains(MachMessageOptions::SEND) {
            KernReturn::INVALID_ARGUMENT.into()
        } else if options.contains(MachMessageOptions::RECEIVE) {
            MACH_RCV_TIMED_OUT
        } else {
            KernReturn::INVALID_ARGUMENT.into()
        }
