...
#[test]
fn live_shared_cache_adoption_is_single_use() {
    let task = task(MacosShimBuilder::new(Platform::new()).build());
    task.global
        .shared_cache_base
    ...
    let cache = LiveSharedCache {
        range: PAGE_SIZE..2 * PAGE_SIZE,
        mappings: &[],
        executable_regions: &[],
        trampoline: LiveSharedCacheTrampoline {
            range: 2 * PAGE_SIZE..3 * PAGE_SIZE,
            writable_alias: PAGE_SIZE,
        },
    };

    assert_eq!(
        program.adopt_live_shared_cache(&cache),
        Err(LiveSharedCacheError::AlreadyAdopted)
    );
}

#[test]
fn privately_mapped_dyld_bootstrap_data_remains_writable() {
...
#[test]
fn sysctl_rejects_unsupported_length_without_reading() {
    use litebox_common_macos::{errno::Errno, user_pointers::UserPtr};

    assert_eq!(
        Task::<Platform>::sys_sysctl_compat(UserPtr::from_usize(1), usize::MAX),
        Err(Errno::ENOSYS)
    );
}
