...
pub struct LinuxKernel<Host: HostInterface> {
    ...
    /// The wall-clock time captured at boot, used together with [`boot_instant`](Self::boot_instant)
...
impl<Host: HostInterface> LinuxKernel<Host> {
    pub fn new(init_page_table_addr: x86_64::PhysAddr) -> &'static Self {
        // Capture the initial wall-clock time and monotonic instant so that
        // subsequent `current_time` calls can be derived from the monotonic
        // clock without additional host calls.
        let boot_system_time = Host::current_realtime();
...
impl<Host: HostInterface> TimeProvider for LinuxKernel<Host> {
    ...
    fn current_time(&self) -> Self::SystemTime {
        // Derive the current wall-clock time from the monotonic clock elapsed
...
pub trait HostInterface: 'static {
    ...
    /// Returns the current wall-clock time as a [`Duration`](core::time::Duration) since the
    /// UNIX epoch.
