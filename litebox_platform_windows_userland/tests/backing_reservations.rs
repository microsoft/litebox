...
use litebox::platform::{
    ...
    page_mgmt::{
        ...
        ReservationOf, ReserveAndCommitError, TrackedReservations,
    },
};
...
#[test]
fn five_primitives_support_default_shortcuts() {
    use core::sync::atomic::{AtomicBool, AtomicUsize, Ordering};
    use litebox::mm::{CreatePagesFlags, LinuxPageManager, NonZeroAddress, NonZeroPageSize};
    use litebox::platform::{
        RawPointerProvider,
        page_mgmt::{AllocationError, PageReservation, PageStateUpdateError},
    };
    ...
    struct Primitives {
        ...
        last_reservation: AtomicUsize,
    }
    impl litebox_platform::sync::RawMutexProvider for Primitives {
        type RawMutex = <WindowsUserland as litebox_platform::sync::RawMutexProvider>::RawMutex;
    }
    impl litebox_platform::sync::WaitWakerProvider for Primitives {}
    impl litebox_platform::time::TimeProvider for Primitives {
        type Instant = <WindowsUserland as litebox_platform::time::TimeProvider>::Instant;
        type SystemTime = <WindowsUserland as litebox_platform::time::TimeProvider>::SystemTime;

        fn now(&self) -> Self::Instant {
            litebox_platform::time::TimeProvider::now(self.native)
        }

        fn current_time(&self) -> Self::SystemTime {
            litebox_platform::time::TimeProvider::current_time(self.native)
        }
    }
    impl RawPointerProvider for Primitives {
        type RawConstPointer<T: FromBytes> =
            <WindowsUserland as RawPointerProvider>::RawConstPointer<T>;
        type RawMutPointer<T: FromBytes + IntoBytes> =
            <WindowsUserland as RawPointerProvider>::RawMutPointer<T>;
    }
    impl PageManagementProvider<4096> for Primitives {
        type Reservations = TrackedReservations<ReservationOf<WindowsUserland, 4096>>;

        const TASK_ADDR_MIN: usize = 0x5000_0000_0000;
        const TASK_ADDR_MAX: usize = Self::TASK_ADDR_MIN + 0x1000_0000;
        const RESERVATION_ALIGNMENT: usize =
            <WindowsUserland as PageManagementProvider<4096>>::RESERVATION_ALIGNMENT;
        ...
        ) -> Result<ReservationOf<Self, 4096>, AllocationError>
        where
            Reservations: Iterator<Item = ReservationOf<Self, 4096>>,
        {
            // SAFETY: The adapter forwards the caller's replacement authorization and ownership.
            let reservation = unsafe {
                <WindowsUserland as PageManagementProvider<4096>>::reserve_pages(
                    self.native,
            ...
            assert_eq!(query(reservation.range().start).State, memory::MEM_RESERVE);
            self.last_reservation
                .store(reservation.range().start, Ordering::Relaxed);
            Ok(reservation)
        }
        unsafe fn reserve_and_commit_pages<Reservations>(
            &self,
            _: impl FnOnce() -> Reservations,
            _: core::ops::Range<usize>,
            _: MemoryRegionPermissions,
            _: bool,
            _: bool,
            _: FixedAddressBehavior,
        ) -> Result<ReservationOf<Self, 4096>, litebox::platform::page_mgmt::ReserveAndCommitError>
        where
            Reservations: Iterator<Item = ReservationOf<Self, 4096>>,
        {
            Err(litebox::platform::page_mgmt::ReserveAndCommitError::UnsupportedByPlatform)
        }
        ...
        where
            Reservations: Iterator<Item = &'reservation ReservationOf<Self, 4096>>,
        {
            for address in range.clone().step_by(4096) {
                assert!(matches!(
                    query(address).State,
            ...
            // SAFETY: The adapter forwards the caller's ownership and access guarantees to the same native provider.
            unsafe {
                <WindowsUserland as PageManagementProvider<4096>>::commit_pages(
                    self.native,
        ...
        {
            assert_eq!(query(range.start).State, memory::MEM_COMMIT);
            // SAFETY: The adapter forwards the caller's committed range and exclusion of conflicting accesses.
            unsafe {
                <WindowsUserland as PageManagementProvider<4096>>::protect_pages(
                    self.native,
                    covering_reservations,
        ...
        where
            Reservations: Iterator<Item = &'reservation ReservationOf<Self, 4096>>,
        {
            // SAFETY: The adapter forwards the caller's covering live handles and exclusion of users.
            unsafe {
                <WindowsUserland as PageManagementProvider<4096>>::decommit_pages(
                    self.native,
                    covering_reservations,
        ...
        unsafe fn release_pages(&self, reservation: ReservationOf<WindowsUserland, 4096>) {
            for address in reservation.range().step_by(4096) {
                assert!(matches!(
                    query(address).State,
                    memory::MEM_RESERVE | memory::MEM_COMMIT
                ));
            }
            // SAFETY: The adapter forwards exclusive ownership of the relinquished extent.
            unsafe {
                <WindowsUserland as PageManagementProvider<4096>>::release_pages(
                    self.native,
                    reservation,
                );
            }
    ...
    let platform = Box::leak(Box::new(Primitives {
        native: WindowsUserland::new(),
        fail_commit: AtomicBool::new(false),
        last_reservation: AtomicUsize::new(0),
    }));
    let manager = LinuxPageManager::<_, 4096>::new(&litebox::LiteBox::new(platform));
    // SAFETY: The test exclusively owns these native allocations and ends all accesses before release.
    unsafe {
        let source = manager
            .create_writable_pages(
                NonZeroAddress::new(Primitives::TASK_ADDR_MIN),
            ...
            .unwrap();
        let extent = source.as_usize()..source.as_usize() + 3 * 4096;
        let reservations = manager.reservations();
        let mappings = manager.mappings();
        platform.fail_commit.store(true, Ordering::Relaxed);
        let error = manager
            .remap_pages(source, 8192, 3 * 4096, true)
            .unwrap_err();
        assert!(matches!(
            error,
        ...
        platform.fail_commit.store(false, Ordering::Relaxed);
        let failed_destination = platform.last_reservation.load(Ordering::Relaxed);
        assert_eq!(query(failed_destination).State, memory::MEM_FREE);
        assert_eq!(
            query(failed_destination + Primitives::RESERVATION_ALIGNMENT - 1).State,
            memory::MEM_FREE
        );
        assert_eq!(query(extent.start).State, memory::MEM_COMMIT);
        assert_eq!(query(extent.start).Protect, memory::PAGE_READWRITE);
        assert_eq!((extent.start as *const u8).read(), 0x5a);
        assert!(matches!(
            reserve_backing(
                platform,
        ...
        manager.make_pages_inaccessible(source, 8192).unwrap();
        assert_eq!(query(extent.start).Protect, memory::PAGE_NOACCESS);
        let moved = manager.remap_pages(source, 8192, 3 * 4096, true).unwrap();
        let destination = manager
            .reservations()
            .into_iter()
            .find(|range| range.contains(&moved.as_usize()))
            .unwrap();
        assert_eq!(destination.len(), Primitives::RESERVATION_ALIGNMENT);
        assert!(
            destination
                .start
                .is_multiple_of(Primitives::RESERVATION_ALIGNMENT)
        );
        assert!(
            destination
                .end
        ...
        let committed = moved.as_usize()..moved.as_usize() + 3 * 4096;
        assert_ne!(destination.start, extent.start);
        manager.unmap_pages(source, extent.len()).unwrap();
        assert_eq!(query(extent.start).State, memory::MEM_FREE);
        assert_eq!(query(committed.start).Protect, memory::PAGE_NOACCESS);
        if committed.end < destination.end {
            assert_eq!(query(committed.end).State, memory::MEM_RESERVE);
        }
        manager.make_pages_readable(moved, committed.len()).unwrap();
        assert_eq!((committed.start as *const u8).read(), 0x5a);
        assert_eq!(((committed.end - 1) as *const u8).read(), 0);
        manager.unmap_pages(moved, committed.len()).unwrap();
        assert_eq!(query(destination.start).State, memory::MEM_FREE);
        assert_eq!(query(destination.end - 1).State, memory::MEM_FREE);

        let shared = manager
            .create_writable_pages(
                NonZeroAddress::new(Primitives::TASK_ADDR_MAX - Primitives::RESERVATION_ALIGNMENT),
                NonZeroPageSize::new(3 * 4096).unwrap(),
                CreatePagesFlags::empty(),
                |pointer| {
                    (pointer.as_usize() as *mut u8).write(0x3c);
                    Ok(0)
                },
            )
            .unwrap();
        let ownership = manager.reservations();
        manager.make_pages_inaccessible(shared, 4096).unwrap();
        let moved = manager.remap_pages(shared, 4096, 8192, true).unwrap();
        assert_eq!(moved.as_usize(), Primitives::TASK_ADDR_MAX - 8192);
        assert_eq!(manager.reservations(), ownership);
        assert_eq!(query(shared.as_usize()).State, memory::MEM_RESERVE);
        assert_eq!(
            query(moved.as_usize()).AllocationBase.addr(),
            shared.as_usize()
        );
        assert_eq!(query(moved.as_usize()).Protect, memory::PAGE_NOACCESS);
        manager.make_pages_readable(moved, 8192).unwrap();
        assert_eq!((moved.as_usize() as *const u8).read(), 0x3c);
        assert_eq!(((moved.as_usize() + 8191) as *const u8).read(), 0);
        manager.unmap_pages(shared, 3 * 4096).unwrap();
        assert_eq!(query(moved.as_usize()).State, memory::MEM_COMMIT);
        manager.unmap_pages(moved, 8192).unwrap();
        assert_eq!(query(shared.as_usize()).State, memory::MEM_FREE);

        let reservation = committed_fixture(
            platform.native,
            0x10000,
            MemoryRegionPermissions::READ | MemoryRegionPermissions::WRITE,
        );
        ...
        let (prefix, middle, suffix) = reservation.split(base + 4096..base + 8192);
        release_reservations::<4096, _>(platform.native, vec![middle]);
        assert_eq!(query(base + 4096).State, memory::MEM_FREE);
        assert_eq!(query(base).State, memory::MEM_COMMIT);
        assert_eq!(query(base + 8192).State, memory::MEM_COMMIT);
        assert_eq!((base as *const u8).read(), 0x5a);
        assert_eq!(((base + 8192) as *const u8).read(), 0xa5);
        for survivor in [prefix.unwrap(), suffix.unwrap()] {
            <WindowsUserland as PageManagementProvider<4096>>::decommit_pages(
                platform.native,
                || core::iter::once(&survivor),
                survivor.range(),
            )
            .unwrap();
            release_reservations::<4096, _>(platform.native, vec![survivor]);
        }
...
unsafe fn release_reservations<const ALIGN: usize, Platform: PageManagementProvider<ALIGN>>(
    ...
    // SAFETY: The fixture relinquishes these exact owned extents without remaining users.
    unsafe {
        for reservation in reservations {
...
fn committed_fixture<Platform: PageManagementProvider<4096>>(
    platform: &Platform,
    length: usize,
    permissions: MemoryRegionPermissions,
) -> ReservationOf<Platform, 4096> {
    ...
    .unwrap();
    // SAFETY: All pages are uncommitted and exclusively owned by the fixture.
    if let Err(error) = unsafe {
        ...
        // SAFETY: Failed commitment leaves this unpublished reservation uncommitted.
        unsafe { release_reservations(platform, vec![reservation]) };
...
fn reserve_backing<Platform: PageManagementProvider<4096>>(
    ...
    // SAFETY: The fixture acquires inaccessible pages without replacing existing mappings.
    unsafe {
        platform.reserve_pages(
            || -> core::iter::Empty<_> {
...
#[test]
fn committed_pages_preserve_contents_and_ownership() {
    let platform = WindowsUserland::new();
    let permissions = MemoryRegionPermissions::READ | MemoryRegionPermissions::WRITE;
    let reservation = committed_fixture(platform, 0x10000, permissions);
    let base = reservation.range().start;
    // SAFETY: The reservation owns this committed extent; all accesses end before release.
    unsafe {
        (base as *mut u8).write(0x5a);
        ((base + 4096) as *mut u8).write(0xa5);
        assert_eq!(query(base).State, memory::MEM_COMMIT);
        assert_eq!(query(base).Protect, memory::PAGE_READWRITE);
        assert_eq!((base as *const u8).read(), 0x5a);
        assert_eq!(((base + 4096) as *const u8).read(), 0xa5);
        assert_eq!(reservation.range(), base..base + 0x10000);
        <WindowsUserland as PageManagementProvider<4096>>::protect_pages(
            platform,
            || core::iter::once(&reservation),
            base..base + 4096,
        ...
        .unwrap();
        assert_eq!(query(base).Protect, memory::PAGE_READONLY);
        assert_eq!((base as *const u8).read(), 0x5a);
        <WindowsUserland as PageManagementProvider<4096>>::decommit_pages(
            platform,
            || core::iter::once(&reservation),
        ...
        .unwrap();
        assert_eq!(((base + 4096) as *const u8).read(), 0);
        <WindowsUserland as PageManagementProvider<4096>>::decommit_pages(
            platform,
            || core::iter::once(&reservation),
            reservation.range(),
...
fn manager_rejects_foreign_backing_without_changing_existing_mapping() {
    ...
    unsafe {
        ...
        explicit.remove_pages(relocated, 4096).unwrap();
        manager.release_memory().unwrap();
        assert_eq!(foreign.cast::<u8>().read(), 0xa5);
    }
}
