...
mod tests {
    ...
    fn regs_for_windows_syscall(
        sysno: NtSysno,
        register_args: [usize; 4],
        stack_args: &[usize],
    ) -> litebox_common_linux::PtRegs {
        litebox_common_linux::PtRegs {
            orig_rax: sysno.as_raw() as usize,
            r10: register_args[0],
            rdx: register_args[1],
            r8: register_args[2],
            r9: register_args[3],
            rsp: (stack_args.as_ptr() as usize)
                .checked_sub(FIRST_STACK_ARGUMENT_OFFSET)
                .expect("test stack address can represent a Windows syscall rsp"),
            ..Default::default()
        }
    }

    #[test]
    fn threadpool_syscall_decode_reads_stack_arguments() {
        type TestSyscallRequest = SyscallRequest<crate::tests::TestPlatform>;

        let associate_register_args = [0x1111_0001, 0x2222_0002, 0x3333_0003, 0x4444_0004];
        let associate_stack_args = [0x5555_0005, 0x0666_0006, 0x7777_0007, 0x8888_0008];
        let associate_regs = regs_for_windows_syscall(
            NtSysno::NtAssociateWaitCompletionPacket,
            associate_register_args,
            &associate_stack_args,
        );
        let associate = TestSyscallRequest::try_from_raw(&associate_regs)
            .expect("NtAssociateWaitCompletionPacket should decode");
        let SyscallRequest::NtAssociateWaitCompletionPacket {
            wait_completion_packet_handle,
            io_completion_handle,
            ...
            panic!("expected NtAssociateWaitCompletionPacket request");
        };
        assert_eq!(
            wait_completion_packet_handle,
            Handle::from_raw(associate_register_args[0])
        );
        assert_eq!(
            io_completion_handle,
            Handle::from_raw(associate_register_args[1])
        );
        assert_eq!(
            target_object_handle,
            Handle::from_raw(associate_register_args[2])
        );
        ...
        assert_eq!(
            already_signaled
                .expect("already_signaled should be non-null")
                .as_usize(),
            associate_stack_args[3]
        );

        let worker_factory_register_args = [0x1010_0001, 0x2020_0002, 0x3030_0003, 0x4040_0004];
        let worker_factory_stack_args = [
            0x5050_0005,
            0x6060_0006,
            0x7070_0007,
            0x8080_0008,
        ...
        let worker_factory_regs = regs_for_windows_syscall(
            NtSysno::NtCreateWorkerFactory,
            worker_factory_register_args,
            &worker_factory_stack_args,
        );
        let worker_factory = TestSyscallRequest::try_from_raw(&worker_factory_regs)
            .expect("NtCreateWorkerFactory should decode");
        let SyscallRequest::NtCreateWorkerFactory {
            worker_factory_handle,
            desired_access,
            object_attributes,
            ...
            panic!("expected NtCreateWorkerFactory request");
        };
        assert_eq!(
        ...
        assert_eq!(
            object_attributes
                ...
                .as_usize(),
            worker_factory_register_args[2]
        );
        assert_eq!(
            completion_port_handle,
            Handle::from_raw(worker_factory_register_args[3])
        );
        assert_eq!(
            worker_process_handle.as_handle(),
            Handle::from_raw(worker_factory_stack_args[0])
        );
        ...
        let timer_register_args = [0x1212_0001, 0x2323_0002, 0x3434_0003, 0x4545_0004];
        let timer_stack_args = [0x5656_0005];
        let timer_regs = regs_for_windows_syscall(
            NtSysno::NtCreateTimer2,
            timer_register_args,
            &timer_stack_args,
        );
        let timer =
            TestSyscallRequest::try_from_raw(&timer_regs).expect("NtCreateTimer2 should decode");
        let SyscallRequest::NtCreateTimer2 {
            timer_handle,
            timer_id,
            object_attributes,
            attributes,
            ...
            panic!("expected NtCreateTimer2 request");
        };
        assert_eq!(timer_handle.as_usize(), timer_register_args[0]);
        assert_eq!(
            timer_id.expect("timer_id should be non-null").as_usize(),
            timer_register_args[1]
        );
        assert_eq!(
            object_attributes
                .expect("object_attributes should be non-null")
    ...
    #[test]
    fn directory_syscall_decode_reads_stack_arguments() {
        type TestSyscallRequest = SyscallRequest<crate::tests::TestPlatform>;

        let create_ex_register_args = [0x1112_0001, 0x2223_0002, 0x3334_0003, 0x4445_0004];
        let create_ex_stack_args = [0x5556_0005];
        let create_ex_regs = regs_for_windows_syscall(
            NtSysno::NtCreateDirectoryObjectEx,
            create_ex_register_args,
            &create_ex_stack_args,
        );
        let create_ex = TestSyscallRequest::try_from_raw(&create_ex_regs)
            .expect("NtCreateDirectoryObjectEx should decode");
        let SyscallRequest::NtCreateDirectoryObjectEx {
            directory_handle,
            desired_access,
            object_attributes,
            ...
            panic!("expected NtCreateDirectoryObjectEx request");
        };
        assert_eq!(directory_handle.as_usize(), create_ex_register_args[0]);
        assert_eq!(
            desired_access,
            u32::try_from(create_ex_register_args[1]).expect("desired_access sentinel fits in u32")
        );
        assert_eq!(
            object_attributes
                .expect("object_attributes should be non-null")
                .as_usize(),
            create_ex_register_args[2]
        );
        assert_eq!(
            shadow_directory_handle,
            Handle::from_raw(create_ex_register_args[3])
        );
        assert_eq!(
            flags,
            u32::try_from(create_ex_stack_args[0]).expect("flags sentinel fits in u32")
        );

        let query_register_args = [0x6161_0001, 0x7272_0002, 0x8383_0003, 0x9494_0004];
        let query_stack_args = [0xa5a5_0005, 0xb6b6_0006, 0xc7c7_0007];
        let query_regs = regs_for_windows_syscall(
            NtSysno::NtQueryDirectoryObject,
            query_register_args,
            &query_stack_args,
        );
        let query = TestSyscallRequest::try_from_raw(&query_regs)
            .expect("NtQueryDirectoryObject should decode");
        let SyscallRequest::NtQueryDirectoryObject {
            directory_handle,
            buffer,
            buffer_length,
            return_single_entry,
            ...
            panic!("expected NtQueryDirectoryObject request");
        };
        assert_eq!(directory_handle, Handle::from_raw(query_register_args[0]));
        assert_eq!(buffer.as_usize(), query_register_args[1]);
        assert_eq!(
            buffer_length,
            u32::try_from(query_register_args[2]).expect("buffer_length sentinel fits in u32")
        );
        assert_eq!(
            return_single_entry,
            u8::try_from(query_register_args[3] & 0xff)
                .expect("return_single_entry sentinel fits in u8")
        );
        assert_eq!(
            restart_scan,
            u8::try_from(query_stack_args[0] & 0xff).expect("restart_scan sentinel fits in u8")
        );
        assert_eq!(context.as_usize(), query_stack_args[1]);
        assert_eq!(
            return_length
                .expect("return_length should be non-null")
                .as_usize(),
            query_stack_args[2]
        );
    }
